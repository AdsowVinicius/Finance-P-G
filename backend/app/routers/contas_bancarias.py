...
def criar_conta_bancaria(dados: ContaBancariaCreate, db: Session = Depends(get_db)) -> ContaBancaria:
...
def atualizar_conta_bancaria(conta_bancaria_id: uuid.UUID, dados: ContaBancariaUpdate, db: Session = Depends(get_db)) -> ContaBancaria:
    conta = db.get(ContaBancaria, conta_bancaria_id)
    if conta is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Conta bancária não encontrada")
...
def desativar_conta_bancaria(conta_bancaria_id: uuid.UUID, db: Session = Depends(get_db)) -> None:
    conta = db.get(ContaBancaria, conta_bancaria_id)
    if conta is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Conta bancária não encontrada")
    conta.ativo = False
